...
def generate_simulation_results(duration_days=365, output_dir="results", seed=42):
    """
    ...
    # Reproducible demo data makes dashboard results stable across refreshes.
    ...
    optimized_energy = baseline_energy * 0.85 + rng.normal(0, 1, len(baseline_energy))
    optimized_energy = np.maximum(optimized_energy, 3)
    
    # Thermal comfort metrics
    pmv_baseline = rng.normal(0.15, 0.35, len(dates))
    pmv_optimized = rng.normal(0.05, 0.25, len(dates))
    
    ppd_baseline = np.maximum(5 + 3 * np.abs(pmv_baseline), 0)
    ppd_optimized = np.maximum(5 + 3 * np.abs(pmv_optimized), 0)
    
    # Zone temperatures
    zone_temps_baseline = 21 + 2.5 * np.sin((hours_array - 12) / 12 * np.pi) + rng.normal(0, 1.2, len(dates))
    zone_temps_optimized = 21 + 2 * np.sin((hours_array - 12) / 12 * np.pi) + rng.normal(0, 0.9, len(dates))
    ...
    for i in range(0, len(dates), 60):  # Every 5 hours, log some actions
        if i < len(dates):
            actions_log.append({
                ...
                "confidence": float(0.85 + rng.random() * 0.1),
    ...
    performance_report = {
        ...
        "data_source": "deterministic_demo_digital_twin",
        "verification_note": "Demo results are generated by the local digital twin. Validate EnergyPlus output before presenting these as measured results.",
